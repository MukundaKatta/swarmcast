...
def main():
    instance = Swarmcast(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced swarmcast Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.optimize(input=inp, index=i) for i, inp in enumerate(inputs)]
